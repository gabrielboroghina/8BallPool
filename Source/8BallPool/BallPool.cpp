...
void BallPool::TryMoveCueBall(const glm::vec2 &move)
{
    ...
    cueBall->Move(glm::vec3(fmax * distX, 0, fmax * distY/* LEN / 6 + RAD - cueBall->pos.z*/));
}
...
void BallPool::ProcessMovements(float deltaTime)
{
    ...
    const auto InsertBallCushionCollision = [&](Ball *ball, int cushionIndex, float timeToCollision) {
        if (timeToCollision <= deltaTime && timeToCollision < firstCollision.t)
            firstCollision = CollisionPair(ball, cushion[cushionIndex], timeToCollision);
    };

    const auto DetectBallChusionCollision = [&](Ball *ball) {
        float vx = ball->velocity.x, vy = ball->velocity.y;
        if (vx < 0) // left cushion
            InsertBallCushionCollision(ball, 0, (-vx - sqrt(pow(vx, 2) + 2 * ACC * (-limX - ball->pos.x))) / ACC);
        if (vx > 0) // right cushion
            InsertBallCushionCollision(ball, 2, (-vx + sqrt(pow(vx, 2) + 2 * ACC * (limX - ball->pos.x))) / ACC);
        if (vy < 0) // top cushion
            InsertBallCushionCollision(ball, 1, (-vy - sqrt(pow(vy, 2) + 2 * ACC * (-limY - ball->pos.z))) / ACC);
        if (vy > 0) // bottom cushion
            InsertBallCushionCollision(ball, 3, (-vy + sqrt(pow(vy, 2) + 2 * ACC * (limY - ball->pos.z))) / ACC);
    };
    ...
    while (firstCollision.t <= deltaTime) {
        ...
        for (auto &ball : balls) ball->Update(firstCollision.t);
        ...
        if (firstCollision.b->type == CUSHION) {
